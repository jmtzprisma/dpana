...
class OrdersController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Iniciar la consulta sin ejecutar 'get()' todavía
        $orders = Order::latest()->byLocation();

        # Filtros condicionales
        if ($request->search != null) {
            $searchKey = $request->search;
            $orders->where(function ($q) use ($searchKey) {
                $customers = User::where('name', 'like', '%' . $searchKey . '%')
                    ->orWhere('phone', 'like', '%' . $searchKey . '%') // Agregar '%' al final
                    ->pluck('id');
        ...
        if ($request->code != null) {
            ...
            $orders->whereIn('order_group_id', $orderGroup);
        }

        if ($request->delivery_status != null) {
            $deliveryStatus = $request->delivery_status;
            $orders->where('delivery_status', $deliveryStatus);
        }
        ...
        $orders->where(function ($q) use ($posOrder) {
            $orderGroup = OrderGroup::where('is_pos_order', $posOrder)->pluck('id');
            $q->orWhereIn('order_group_id', $orderGroup);
        });

        // Clonar la consulta para 'deliver_orders'
        $deliver_orders = clone $orders;
        $deliver_orders->where('user_in_location', true);

        // Ejecutar la consulta aquí
        $orders = $orders->paginate(paginationNumber());
        $deliver_orders = $deliver_orders->paginate(paginationNumber());
        ...
        return view('backend.pages.orders.index', compact(
            'orders',
            'deliver_orders',
            'searchKey',
            'locations',
