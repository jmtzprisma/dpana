...
class SettingsController extends Controller
{
    ...
    #limits credit
    public function limits_credit()
    {
        return view('backend.pages.systemSettings.limits_credit');
    }
    
    public function limits_credit_save(Request $request)
    {
        $arr = array();
        for($x = 0; $x < count($request->limit['limit']); $x++)
        {
            if(!is_null($request->limit['from'][$x]))
                $arr[] = array(
                    'from' => $request->limit['from'][$x],
                    'to' => $request->limit['to'][$x],
                    'limit' => (float)$request->limit['limit'][$x],
                );
        }
        
        $setting = SystemSetting::where('entity', 'credit_limits')->first();
        if ($setting != null) {
            $setting->value = json_encode($arr);
            $setting->save();
        } else {
            $setting = new SystemSetting;
            $setting->entity = 'credit_limits';
            $setting->value = json_encode($arr);
            $setting->save();
        }
        cacheClear();

        flash(localize("Limites actualizados correctamente"))->success();
        return back();
    }

    public function num_cuotas_save(Request $request)
    {
        
        $setting = SystemSetting::where('entity', 'num_cuotas')->first();
        if ($setting != null) {
            $setting->value = $request->num_cuotas;
            $setting->save();
        } else {
            $setting = new SystemSetting;
            $setting->entity = 'num_cuotas';
            $setting->value = $request->num_cuotas;
            $setting->save();
        }
        cacheClear();

        flash(localize("Número de cuotas actualizado correctamente"))->success();
        return back();
    }
