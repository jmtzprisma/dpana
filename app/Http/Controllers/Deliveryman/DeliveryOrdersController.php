...
class DeliveryOrdersController extends Controller
{
    ...
    public function assigned(Request $request)
    {
        ...
        // Filtrar las órdenes
        $orders = Order::where('deliveryman_id', $user->id)
            ->where(function ($q) {
        ...
        // Si el usuario no es admin, filtrar por su ubicación
        if (!$isAdmin) {
            $orders = $orders->where('location_id', $userLocationId);
        }

        // Aplicar filtros de búsqueda opcionales
        ...
        return view('deliveryman.assigned', compact('orders', 'searchKey', 'searchCode', 'paymentStatus', 'locations')); 
    }
